          ...
          <div className="fu" style={{ background:T.accentDim, border:`1px solid ${T.accent}33`, borderRadius:10, padding:10, marginBottom:10 }}>
            ...
            <div style={{ display:"flex", gap:6 }}>
              ...
              <button onClick={addEvent} disabled={saving}
                style={{ padding:"6px 14px", background:`linear-gradient(135deg,${T.accent},${T.pink})`, borderRadius:7, color:"#111111", fontSize:12, fontWeight:700, opacity:saving?0.6:1 }}>Save</button>
            </div>
          </div>
        )}
...
function DailyBibleChapter() {
  ...
  return (
    <div style={{ padding:"12px", borderRadius:12, background:`linear-gradient(135deg,rgba(12,12,12,0.95),rgba(25,35,5,0.7))`, border:`1px solid ${T.accent}44` }}>
...
function PomodoroTimer() {
  ...
  return (
    <div style={{ background:`linear-gradient(145deg,#1e1e1e,${T.surface})`, borderRadius:14, padding:"18px", border:`1px solid ${T.accent}33`, boxShadow:`0 0 20px ${T.accentGlow}`, display:"flex", flexDirection:"column", height:"100%" }}>
      {/* Header */}
        ...
        {pomState==="idle"?(
          <button onClick={()=>{ setPomSecs(customMin*60); setPomState("work"); }}
            style={{ flex:2, padding:"10px", borderRadius:10, fontWeight:700, fontSize:13,
              background:`linear-gradient(135deg,${T.accent},${T.accentB})`, border:"none", color:"#111111",
              boxShadow:`0 4px 14px rgba(200,241,53,0.3)` }}>▶ Start Focus</button>
        ):(
...
export default function Dashboard() {
  ...
  const [newGoal,     setNewGoal]     = useState({ text:"", progress:0, color:"#c8f135" });
  ...
  const [newBook,     setNewBook]     = useState({ title:"", author:"", pages_total:0, pages_read:0, cover_color:"#c8f135" });
  ...
  const addGoal     = async() => { if(!newGoal.text.trim()) return; setSaving(true); const{data}=await supabase.from("goals").insert({text:newGoal.text,progress:newGoal.progress,color:newGoal.color}).select().single(); if(data) setGoals(gs=>[...gs,data]); setNewGoal({text:"",progress:0,color:"#c8f135"}); setShowGoalForm(false); setSaving(false); showToast("Goal added ✓"); };
  ...
  return (
    <div className="tf-app" style={{ background:T.bg, fontFamily:"'Plus Jakarta Sans',sans-serif", color:T.text, fontSize:13 }}>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@300;400;500;600;700;800&family=Bebas+Neue&family=Cormorant+Garamond:ital,wght@1,300;1,400&display=swap');        *{box-sizing:border-box;margin:0;padding:0;scrollbar-width:thin;scrollbar-color:#282828 transparent}
        ::-webkit-scrollbar{width:3px}::-webkit-scrollbar-thumb{background:#323232;border-radius:2px}
        html,body,#root{height:100%;width:100%}
        input,select,button,textarea{font-family:'Plus Jakarta Sans',sans-serif}
        input::placeholder{color:#323232}input:focus,select:focus{outline:none}
        button{cursor:pointer;border:none;outline:none}
        .nb:hover{background:rgba(200,241,53,0.10)!important}
        .task-row:hover{background:rgba(200,241,53,0.06)!important}
        ...
        .tf-hamburger{display:none;background:#1e1e1e;border:1px solid #323232;color:#6b6b9a;width:34px;height:34px;border-radius:8px;font-size:16px;align-items:center;justify-content:center;cursor:pointer;flex-shrink:0}
          ...
          <div className="fu" style={{ background:T.accentDim, border:`1px solid ${T.accent}33`, borderRadius:10, padding:10 }}>
            ...
            <div style={{ display:"flex", gap:6 }}>
              <select value={newBook.cover_color} onChange={e=>setNewBook({...newBook,cover_color:e.target.value})}
                ...
                <option value="#c8f135">🟣 Purple</option>
                <option value="#10b981">🟢 Green</option>
                <option value="#aadc1a">🟡 Yellow</option>
      ...
      <div className="tf-main">
        ...
        <div ref={contentRef} className="tf-content" style={{ display:"flex", flexDirection:"column", gap:14 }}>
          ...
          <div className="tf-row1" style={{ alignItems:"stretch" }}>
            ...
            {/* Weekly Bar */}
            <div style={{ background:`linear-gradient(145deg,#1e1e1e,${T.surface})`, borderRadius:14, padding:"16px 18px", border:`1px solid ${T.accent}44`, boxShadow:`0 0 20px ${T.accentGlow}` }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12 }}>
                <div>
              ...
              {(()=>{
                ...
                // Get real current week starting Monday
                const now = new Date();
                ...
                const weekDateLabels = dayNames.map((_,i)=>{
                  const d = new Date(monday);
                  d.setDate(monday.getDate()+i);
                ...
                const weekBars = dayNames.map((day,i)=>{
                  ...
                  return { day, done:dayTasks.length, isToday:i===todayIdx, taskNames:dayTasks.map(t=>t.text), dateLabel:weekDateLabels[i] };
                });
                    ...
                    <div style={{ display:"grid", gridTemplateColumns:"repeat(7,1fr)", gap:5, marginTop:8 }}>
                      {weekBars.map((d,i)=>(
                        <div key={i} style={{ padding:"8px 5px", borderRadius:9, minHeight:80,
                          background: d.isToday ? T.accentDim : d.done>0 ? "rgba(200,241,53,0.06)" : "rgba(255,255,255,0.02)",
                          border:`1px solid ${d.isToday?T.accent+"44":d.done>0?"rgba(200,241,53,0.12)":T.border}` }}>
                    ...
                    {(()=>{
                      ...
                      return (
                        <div style={{ marginTop:10, padding:"10px 14px", borderRadius:10,
                          background:streak>0?`linear-gradient(135deg,rgba(200,241,53,0.12),rgba(236,72,153,0.08))`:"rgba(255,255,255,0.02)",
            ...
            {/* Weather */}
            <div style={{ background:`linear-gradient(145deg,#1e1e1e,${T.surface})`, borderRadius:14, padding:"14px 16px", border:`1px solid ${T.accent}33`, overflow:"hidden", boxShadow:`0 0 20px ${T.accentGlow}` }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
            ...
            {/* MY CARDS */}
            <div style={{ background:"linear-gradient(160deg,#1e1e1e 0%,#1e1e0a 50%,#1e1e1e 100%)", borderRadius:14, padding:"16px 18px", border:`1px solid rgba(200,241,53,0.22)`, display:"flex", flexDirection:"column", gap:10, boxShadow:`0 8px 32px rgba(200,241,53,0.12)` }}>
                    ...
                    return (
                      <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                        ...
                        <div style={{ borderRadius:14, overflow:"hidden", position:"relative",
                          background:`linear-gradient(135deg,${c.color||"#282828"}cc 0%, #1e1e1e 60%,#111111 100%)`,
                          border:"1px solid rgba(255,255,255,0.12)", boxShadow:"0 8px 32px rgba(0,0,0,0.5)" }}>
                          <div style={{ position:"absolute", top:-30, right:-30, width:100, height:100, borderRadius:"50%", background:"rgba(200,241,53,0.12)", pointerEvents:"none" }} />
          ...
          <div className="tf-row2" style={{ minHeight:"auto" }}>
            ...
            <div id="section-tasks" style={{ background:T.surface, borderRadius:14, padding:"18px 18px", border:`1px solid ${T.border}`, display:"flex", flexDirection:"column" }}>
              ...
              <div style={{ display:"flex", gap:7, marginBottom:12 }}>
                ...
                <button onClick={addTask} disabled={saving} style={{ padding:"8px 14px", background:`linear-gradient(135deg,${T.accent},${T.pink})`, borderRadius:9, color:"#111111", fontSize:16, fontWeight:900, boxShadow:`0 0 12px ${T.accentGlow}`, opacity:saving?0.6:1 }}>+</button>
              </div>
                ...
                <div className="fu" style={{ background:T.accentDim, border:`1px solid ${T.accent}33`, borderRadius:10, padding:11, marginBottom:12 }}>
                  ...
                  <div style={{ display:"flex", gap:6 }}>
                    ...
                    <select value={newGoal.color} onChange={e=>setNewGoal({...newGoal,color:e.target.value})}
                      ...
                      <option value="#ec4899">🩷 Pink</option>
                      <option value="#ef4444">🔴 Red</option>
                      <option value="#d4f55a">💜 Lavender</option>
                    </select>
            ...
            <div style={{ background:T.surface, borderRadius:14, padding:"18px 18px", border:`1px solid ${T.border}`, display:"flex", flexDirection:"column", gap:8 }}>
              ...
              {!fitnessLog ? <Spinner/> : (()=>{
                const metrics = [
                  ...
                  { key:"sleep_hrs", label:"Sleep",    icon:"😴", unit:"hrs",   max:fitnessGoals.sleep_hrs, color:"#d4f55a" },
              ...
              {showTxnForm&&(
                <div className="fu" style={{ background:T.accentDim, border:`1px solid ${T.accent}33`, borderRadius:10, padding:11, marginBottom:11 }}>
                  ...
                  <div style={{ display:"flex", gap:6 }}>
                    ...
                    <button onClick={addTxn} disabled={saving} style={{ padding:"7px 12px", background:`linear-gradient(135deg,${T.accent},${T.pink})`, borderRadius:7, color:"#111111", fontSize:12, fontWeight:700, opacity:saving?0.6:1 }}>Save</button>
                  </div>
                </div>
              )}
        ...
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.8)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:1000 }}
          ...
          <div className="fu" style={{ background:T.card, border:`1px solid ${T.border2}`, borderRadius:16, padding:26, width:310, boxShadow:"0 28px 60px rgba(0,0,0,0.8)" }}>
            ...
            <div style={{ display:"flex", gap:9, marginTop:18 }}>
              ...
              <button onClick={()=>saveGoal(editGoal,editGoal.progress)} style={{ flex:1, padding:"10px", background:`linear-gradient(135deg,${T.accent},${T.pink})`, borderRadius:9, color:"#111111", fontSize:13, fontWeight:700, boxShadow:`0 0 14px ${T.accentGlow}` }}>Save</button>
            </div>
          </div>
        </div>
      )}
