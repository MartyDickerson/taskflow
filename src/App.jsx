...
/* ── Data ────────────────────────────────────────────────────────────── */
const INIT_TASKS = [
  { id:1, text:"Review Q2 budget report",    done:true,  priority:"high" },
  { id:2, text:"Team standup at 10am",        done:true,  priority:"medium" },
  { id:3, text:"Update project roadmap",      done:false, priority:"high" },
  { id:4, text:"Send client proposal draft",  done:false, priority:"high" },
  { id:5, text:"Code review — PR #42",        done:false, priority:"medium" },
  { id:6, text:"Fix nav bug on mobile",       done:true,  priority:"low" },
  { id:7, text:"Write weekly retrospective",  done:false, priority:"low" },
];

const INIT_GOALS = [
  { id:1, text:"Ship v2.0 feature",     progress:75, color:T.orange },
  { id:2, text:"Write 3 blog posts",    progress:33, color:T.green },
  { id:3, text:"Exercise 4× this week", progress:50, color:T.yellow },
  { id:4, text:"Read 100 pages",        progress:90, color:"#a78bfa" },
];

const TXNS = [
  { id:1, name:"Salary Deposit",    amount:+4200,  icon:"💼", date:"Today, 9:00 AM",    type:"income" },
  { id:2, name:"Netflix",           amount:-15.99, icon:"N",  date:"Yesterday, 11:30 PM",type:"expense" },
  { id:3, name:"Grocery Store",     amount:-87.40, icon:"🛒", date:"May 17, 3:15 PM",   type:"expense" },
  { id:4, name:"Freelance Payment", amount:+850,   icon:"💻", date:"May 16, 2:00 PM",   type:"income" },
  { id:5, name:"Electric Bill",     amount:-92.00, icon:"⚡", date:"May 15, 8:45 AM",   type:"expense" },
];
...
const NAV_ICONS = { Dashboard:"⊞", Tasks:"✓", Calendar:"📅", Finance:"◈", Goals:"🎯", Settings:"⚙", "Help Center":"?" };
...
const CustomTip = ({ active, payload }) => {
  ...
  return (
    <div style={{ background:T.card, border:`1px solid ${T.border2}`, borderRadius:9, padding:"9px 13px", fontSize:12 }}>
      ...
      {payload[1] && <div style={{ color:T.faint, fontWeight:600, marginTop:2 }}>Total: {payload[0]?.payload?.total}</div>}
    </div>
  );
};
  ...
  return (
    <div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(7,1fr)", gap:2, marginBottom:4 }}>
        {DAYS_S.map(d=><div key={d} style={{ textAlign:"center", fontSize:9, color:T.faint, fontWeight:700,
          padding:"2px 0", textTransform:"uppercase", letterSpacing:0.5 }}>{d}</div>)}
      </div>
...
export default function Dashboard() {
  ...
  const deleteTask = id => { setTasks(ts=>ts.filter(t=>t.id!==id)); showToast("Task removed"); };
  const addTask    = () => {
    if (!newTask.trim()) return;
    setTasks(ts=>[...ts,{id:Date.now(),text:newTask.trim(),done:false,priority:"medium"}]);
    setNewTask(""); showToast("Task added ✓");
  };
  const saveGoal = (g,pct) => {
    setGoals(gs=>gs.map(x=>x.id===g.id?{...x,progress:pct}:x));
  ...
  const expenses = txns.filter(t=>t.type==="expense").reduce((a,t)=>a+Math.abs(t.amount),0);
  ...
  return (
    <div style={{ display:"flex", height:"100vh", background:T.bg, fontFamily:"'Plus Jakarta Sans',sans-serif",
      color:T.text, overflow:"hidden", fontSize:13 }}>
      <style>{`
      ...
      <div style={{ width:218, background:T.surface, borderRight:`1px solid ${T.border}`,
        ...
        <div style={{ display:"flex", alignItems:"center", gap:10, padding:"0 8px", marginBottom:28 }}>
          ...
          <span style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:22, letterSpacing:"2px", color:T.text }}>TaskFlow</span>
          ...
          {NAV.slice(0,5).map(item=>{
            ...
            return (
              <div key={item} className="nb" onClick={()=>setActiveNav(item)} style={{
                ...
                <span style={{ fontSize:14, color:active?T.orange:T.faint }}>{NAV_ICONS[item]}</span>
                <span style={{ fontSize:13,fontWeight:active?700:400,color:active?T.text:T.muted }}>{item}</span>
              </div>
            );
          })}
          <div style={{ fontSize:9,color:T.faint,fontWeight:800,letterSpacing:"1.4px",
            textTransform:"uppercase",padding:"16px 9px 6px",marginBottom:2 }}>TOOLS</div>
      ...
      <div style={{ flex:1, display:"flex", flexDirection:"column", overflow:"hidden" }}>
        ...
        <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",
          ...
          <div>
            ...
            <div style={{ fontSize:12,color:T.muted,marginTop:2 }}>{tasks.filter(t=>!t.done).length} tasks remaining · Mon, May 18 2026</div>
          </div>
          <div style={{ display:"flex",gap:9,alignItems:"center" }}>
            {[{l:"+ New Task",primary:true},{l:"⬆ Export",primary:false}].map(b=>(
              <button key={b.l} style={{ display:"flex",alignItems:"center",gap:6,padding:"8px 16px",
                borderRadius:9,fontSize:13,fontWeight:700,
                background:b.primary?T.orange:T.raised,
                border:b.primary?"none":`1px solid ${T.border2}`,
                color:b.primary?"white":T.muted,
                boxShadow:b.primary?`0 0 16px ${T.orangeGlow}`:"none",
                transition:"all 0.15s" }}
                onClick={()=>b.primary&&document.getElementById("newTaskInput")?.focus()}>
                {b.l}
              </button>
            ))}
        ...
        <div style={{ flex:1,overflow:"auto",padding:"18px 22px",display:"flex",flexDirection:"column",gap:14 }}>
          ...
          {/* ── ROW 1: Completion Ring + Weekly Bar + Finance Cards ── */}
          <div style={{ display:"grid",gridTemplateColumns:"auto 1fr auto auto",gap:14,alignItems:"stretch" }}>
            ...
            <div style={{ background:T.surface,borderRadius:14,padding:"18px 20px",
              border:`1px solid ${T.border}`,display:"flex",flexDirection:"column",justifyContent:"space-between",width:210 }}>
              ...
              <div style={{ display:"flex",alignItems:"center",justifyContent:"center",flex:1 }}>
                <Ring pct={pct} color={T.orange} center={
                  <div style={{ textAlign:"center" }}>
                    <div style={{ fontFamily:"'Bebas Neue',sans-serif",fontSize:30,letterSpacing:"1px",color:T.text,lineHeight:1 }}>{pct}%</div>
            ...
            <div style={{ background:T.surface,borderRadius:14,padding:"18px 20px",border:`1px solid ${T.border}` }}>
              ...
              <ResponsiveContainer width="100%" height={110}>
                <BarChart data={WEEK_BARS} barSize={18} margin={{top:4,right:4,left:-24,bottom:0}}>
                  <XAxis dataKey="day" tick={{fill:T.faint,fontSize:11,fontFamily:"'Plus Jakarta Sans'"}} axisLine={false} tickLine={false}/>
                  ...
                  <Bar dataKey="done" radius={[5,5,0,0]}>
                    {WEEK_BARS.map((e,i)=><Cell key={i} fill={i===0?T.orange:T.faint}/>)}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            </div>
            ...
            <div style={{ background:`linear-gradient(145deg,#1a0e06,#131313)`,borderRadius:14,
              padding:"18px 20px",border:`1px solid ${T.orange}33`,position:"relative",
            ...
            {/* Spending Donut */}
            <div style={{ background:T.surface,borderRadius:14,padding:"18px 20px",border:`1px solid ${T.border}`,width:200 }}>
              <div style={{ fontSize:10,color:T.muted,fontWeight:700,letterSpacing:"1px",
          ...
          {/* ── ROW 2: To-Do + Goals + Calendar + Finance ── */}
          <div style={{ display:"grid",gridTemplateColumns:"1.1fr 0.9fr 0.85fr 0.9fr",gap:14,flex:1,minHeight:0 }}>
              ...
              <div style={{ flex:1,overflowY:"auto",display:"flex",flexDirection:"column",gap:3 }}>
                {tasks.map(task=>(
                  <div key={task.id} className="task-row fu" style={{ display:"flex",alignItems:"center",
                    ...
                    <div onClick={()=>toggleTask(task.id)} style={{ width:17,height:17,borderRadius:5,
                      flexShrink:0,marginRight:9,border:`2px solid ${task.done?T.green:T.faint}`,
                      background:task.done?T.green:"transparent",display:"flex",alignItems:"center",
                    ...
                    <span onClick={()=>toggleTask(task.id)} style={{ fontSize:12.5,flex:1,
            ...
            <div style={{ background:T.surface,borderRadius:14,padding:"18px 18px",
              ...
              <div style={{ display:"flex",alignItems:"center",gap:14,marginBottom:16,
                ...
                <Ring pct={Math.round(goals.reduce((a,g)=>a+g.progress,0)/goals.length)} size={60} stroke={6}
                  color={T.orange} center={
                ...
                <div>
                  ...
                  <div style={{ fontSize:11,color:T.muted }}>{goals.filter(g=>g.progress===100).length} of {goals.length} goals complete</div>
                </div>
              </div>
                    ...
                    <div style={{ display:"flex",justifyContent:"space-between",marginBottom:6,alignItems:"center" }}>
                      ...
                      <div style={{ display:"flex",alignItems:"center",gap:6 }}>
                        ...
                        <button onClick={()=>setEditGoal({...g})}
                          style={{ background:"none",fontSize:11,color:T.muted,padding:"1px 4px",
              ...
              <div style={{ marginTop:14,paddingTop:12,borderTop:`1px solid ${T.border}` }}>
                <div style={{ fontSize:10,color:T.muted,fontWeight:700,letterSpacing:"1px",
                  textTransform:"uppercase",marginBottom:8 }}>Budget Used</div>
                <div style={{ display:"flex",justifyContent:"space-between",marginBottom:6 }}>
                  <span style={{ fontSize:12,color:T.muted }}>$2,455 / $3,000</span>
                  <span style={{ fontSize:12,fontWeight:700,color:T.orange }}>82%</span>
                </div>
                <ProgressBar pct={82} color={T.orange} h={5} />
              </div>
            </div>
            ...
            <div style={{ background:T.surface,borderRadius:14,padding:"18px 18px",
              border:`1px solid ${T.border}`,display:"flex",flexDirection:"column" }}>
              <div style={{ fontSize:10,color:T.muted,fontWeight:700,letterSpacing:"1px",
                textTransform:"uppercase",marginBottom:13 }}>Finance Tracker</div>

              {/* Mini stats */}
              <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:13 }}>
                {[{l:"Income",v:`$${income.toFixed(0)}`,c:T.green,i:"⬆"},{l:"Spent",v:`$${expenses.toFixed(0)}`,c:T.red,i:"⬇"}].map(s=>(
                  <div key={s.l} style={{ padding:"10px 12px",background:T.raised,borderRadius:10,
                    border:`1px solid ${T.border2}` }}>
                    <div style={{ display:"flex",alignItems:"center",gap:5,marginBottom:4 }}>
                      <span style={{ fontSize:12,color:s.c }}>{s.i}</span>
                      <span style={{ fontSize:10,color:T.muted }}>{s.l}</span>
                    </div>
                    <div style={{ fontFamily:"'Bebas Neue',sans-serif",fontSize:19,letterSpacing:"0.5px",color:s.c }}>{s.v}</div>
                  </div>
                ))}
              </div>
                ...
                {txns.map(tx=>(
                  <div key={tx.id} className="txn-row" style={{ display:"flex",alignItems:"center",gap:9,
                    ...
                    <div style={{ flex:1,minWidth:0 }}>
                      <div style={{ fontSize:12,fontWeight:600,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap" }}>{tx.name}</div>
                      <div style={{ fontSize:10,color:T.muted }}>{tx.date}</div>
                    </div>
                    <div style={{ fontSize:13,fontWeight:800,flexShrink:0,
                      color:tx.amount>0?T.green:T.red }}>
              ...
              <div style={{ marginTop:12,padding:"10px 12px",borderRadius:10,
                ...
                <div style={{ display:"flex",justifyContent:"space-between",fontSize:11,marginBottom:5 }}>
                  ...
                  <span style={{ fontWeight:700,color:T.orange }}>
                    {Math.round((expenses/3000)*100)}% used
                  </span>
                </div>
        ...
        <div style={{ position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",display:"flex",
          ...
          <div className="fu" style={{ background:T.card,border:`1px solid ${T.border2}`,
            ...
            <div style={{ textAlign:"center",fontFamily:"'Bebas Neue',sans-serif",fontSize:36,
              ...
              textShadow:`0 0 20px ${editGoal.color}66` }}>
              {editGoal.progress}%
            </div>
            <ProgressBar pct={editGoal.progress} color={editGoal.color} h={6} />
            <div style={{ display:"flex",gap:9,marginTop:18 }}>
              <button onClick={()=>setEditGoal(null)}
                style={{ flex:1,padding:"10px",background:T.raised,border:`1px solid ${T.border2}`,
                  borderRadius:9,color:T.muted,fontSize:13,fontWeight:600 }}>Cancel</button>
