...
function HabitTracker() {
    ...
    const load = async() => {
      ...
      if(hData?.length) {
        ...
        const [{ data: lData }, { data: sData }] = await Promise.all([
          supabase.from("habit_logs").select("*").eq("log_date", today).in("habit_id", ids),
          supabase.from("habit_streaks").select("*").in("habit_id", ids),
        ]);
        const logMap = {};
        (lData||[]).forEach(l=>{ logMap[l.habit_id] = l; });
        setLogs(logMap);
        const streakMap = {};
        (sData||[]).forEach(s=>{ streakMap[s.habit_id] = s.streak; });
  ...
  const toggle = async(habit) => {
    ...
    if(existing){
      setLogs(l=>({...l,[habit.id]:{...existing,completed:newVal,done:newVal}}));
      await supabase.from("habit_logs").update({completed:newVal,done:newVal}).eq("id",existing.id);
    } else {
      const {data} = await supabase.from("habit_logs").insert({habit_id:habit.id,log_date:today,completed:true,done:true}).select().single();
      if(data) setLogs(l=>({...l,[habit.id]:data}));
    }
    // Refresh streaks
    const { data: sData } = await supabase.from("habit_streaks").select("*").in("habit_id", habits.map(h=>h.id));
    const streakMap = {};
    (sData||[]).forEach(s=>{ streakMap[s.habit_id] = s.streak; });
    setStreaks(streakMap);
  };
  ...
  const done = habits.filter(h=>logs[h.id]?.completed||logs[h.id]?.done).length;
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:8, flex:1 }}>
      ...
      {/* Habit list */}
      {habits.map(h=>{
        const completed = logs[h.id]?.completed || logs[h.id]?.done;
        ...
        return (
          <div key={h.id} onClick={()=>toggle(h)}
            ...
            <div style={{ width:26, height:26, borderRadius:8, display:"flex", alignItems:"center", justifyContent:"center",
              ...
              <span style={{ fontSize:13 }}>{completed?"✓":h.icon}</span>
            </div>
            {/* Label */}
            ...
            {/* Streak */}
            {streak>0&&(
              <div style={{ display:"flex", alignItems:"center", gap:3, padding:"2px 7px", borderRadius:20,
                background:streak>=7?"rgba(251,191,36,0.15)":streak>=3?"rgba(249,115,22,0.12)":"rgba(124,58,237,0.1)",
                border:`1px solid ${streak>=7?"#fbbf2444":streak>=3?"#f9731644":T.accent+"33"}` }}>
                <span style={{ fontSize:10 }}>🔥</span>
                <span style={{ fontSize:10, fontWeight:700,
                  color:streak>=7?"#fbbf24":streak>=3?"#f97316":T.accentLight }}>{streak}</span>
              </div>
            )}
