...
function DailyBibleChapter() {
  ...
  return (
    <div style={{ padding:"12px", borderRadius:12, background:`linear-gradient(135deg,rgba(10,10,10,0.95),rgba(30,30,10,0.7))`, border:`1px solid ${T.accent}44` }}>
...
function PomodoroTimer() {
  ...
  return (
    <div style={{ background:`linear-gradient(145deg,#1a1a1a,${T.surface})`, borderRadius:14, padding:"18px", border:`1px solid ${T.accent}33`, boxShadow:`0 0 20px ${T.accentGlow}`, display:"flex", flexDirection:"column", height:"100%" }}>
      {/* Header */}
...
export default function Dashboard() {
  ...
  const [newGoal,     setNewGoal]     = useState({ text:"", progress:0, color:"#f5c518" });
  ...
  const [newBook,     setNewBook]     = useState({ title:"", author:"", pages_total:0, pages_read:0, cover_color:"#f5c518" });
  ...
  const addGoal     = async() => { if(!newGoal.text.trim()) return; setSaving(true); const{data}=await supabase.from("goals").insert({text:newGoal.text,progress:newGoal.progress,color:newGoal.color}).select().single(); if(data) setGoals(gs=>[...gs,data]); setNewGoal({text:"",progress:0,color:"#f5c518"}); setShowGoalForm(false); setSaving(false); showToast("Goal added ✓"); };
  ...
  return (
    <div className="tf-app" style={{ background:T.bg, fontFamily:"'Plus Jakarta Sans',sans-serif", color:T.text, fontSize:13 }}>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@300;400;500;600;700;800&family=Bebas+Neue&family=Cormorant+Garamond:ital,wght@1,300;1,400&display=swap');        *{box-sizing:border-box;margin:0;padding:0;scrollbar-width:thin;scrollbar-color:#2a2a2a transparent}
        ::-webkit-scrollbar{width:3px}::-webkit-scrollbar-thumb{background:#333333;border-radius:2px}
        ...
        .nb:hover{background:rgba(245,197,24,0.10)!important}
        .task-row:hover{background:rgba(245,197,24,0.06)!important}
        ...
        .tf-hamburger{display:none;background:#1e1e1e;border:1px solid #333333;color:#6b6b9a;width:34px;height:34px;border-radius:8px;font-size:16px;align-items:center;justify-content:center;cursor:pointer;flex-shrink:0}
          ...
          <div className="fu" style={{ background:T.accentDim, border:`1px solid ${T.accent}33`, borderRadius:10, padding:10 }}>
            ...
            <div style={{ display:"flex", gap:6 }}>
              <select value={newBook.cover_color} onChange={e=>setNewBook({...newBook,cover_color:e.target.value})}
                ...
                <option value="#f5c518">🟣 Purple</option>
                <option value="#10b981">🟢 Green</option>
                <option value="#e6b800">🟡 Yellow</option>
              ...
              <button onClick={async()=>{
                ...
                setNewBook({title:"",author:"",pages_total:0,pages_read:0,cover_color:"#f5c518"});
      ...
      <div className="tf-main">
        ...
        <div ref={contentRef} className="tf-content" style={{ display:"flex", flexDirection:"column", gap:14 }}>
          ...
          <div className="tf-row1" style={{ alignItems:"stretch" }}>
            ...
            {/* Weekly Bar */}
            <div style={{ background:`linear-gradient(145deg,#1a1a1a,${T.surface})`, borderRadius:14, padding:"16px 18px", border:`1px solid ${T.accent}44`, boxShadow:`0 0 20px ${T.accentGlow}` }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12 }}>
                <div>
                    ...
                    <div style={{ display:"grid", gridTemplateColumns:"repeat(7,1fr)", gap:5, marginTop:8 }}>
                      {weekBars.map((d,i)=>(
                        <div key={i} style={{ padding:"8px 5px", borderRadius:9, minHeight:80,
                          background: d.isToday ? T.accentDim : d.done>0 ? "rgba(245,197,24,0.06)" : "rgba(255,255,255,0.02)",
                          border:`1px solid ${d.isToday?T.accent+"44":d.done>0?"rgba(245,197,24,0.12)":T.border}` }}>
                    ...
                    {(()=>{
                      ...
                      return (
                        <div style={{ marginTop:10, padding:"10px 14px", borderRadius:10,
                          background:streak>0?`linear-gradient(135deg,rgba(245,197,24,0.12),rgba(236,72,153,0.08))`:"rgba(255,255,255,0.02)",
            ...
            {/* Weather */}
            <div style={{ background:`linear-gradient(145deg,#1a1a1a,${T.surface})`, borderRadius:14, padding:"14px 16px", border:`1px solid ${T.accent}33`, overflow:"hidden", boxShadow:`0 0 20px ${T.accentGlow}` }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
            ...
            {/* MY CARDS */}
            <div style={{ background:"linear-gradient(160deg,#1a1a1a 0%,#3a3a00 50%,#1a1a1a 100%)", borderRadius:14, padding:"16px 18px", border:`1px solid rgba(245,197,24,0.25)`, display:"flex", flexDirection:"column", gap:10, boxShadow:`0 8px 32px rgba(245,197,24,0.15)` }}>
                    ...
                    return (
                      <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                        ...
                        <div style={{ borderRadius:14, overflow:"hidden", position:"relative",
                          background:`linear-gradient(135deg,${c.color||"#2a2a2a"}cc 0%, #1e1e1e 60%,#0f0f0f 100%)`,
                          border:"1px solid rgba(255,255,255,0.12)", boxShadow:"0 8px 32px rgba(0,0,0,0.5)" }}>
                          <div style={{ position:"absolute", top:-30, right:-30, width:100, height:100, borderRadius:"50%", background:"rgba(245,197,24,0.12)", pointerEvents:"none" }} />
                ...
                <div className="fu" style={{ background:T.accentDim, border:`1px solid ${T.accent}33`, borderRadius:10, padding:11, marginBottom:12 }}>
                  ...
                  <div style={{ display:"flex", gap:6 }}>
                    ...
                    <select value={newGoal.color} onChange={e=>setNewGoal({...newGoal,color:e.target.value})}
                      ...
                      <option value="#ec4899">🩷 Pink</option>
                      <option value="#ef4444">🔴 Red</option>
                      <option value="#fdd835">💜 Lavender</option>
                    </select>
            ...
            <div style={{ background:T.surface, borderRadius:14, padding:"18px 18px", border:`1px solid ${T.border}`, display:"flex", flexDirection:"column", gap:8 }}>
              ...
              {!fitnessLog ? <Spinner/> : (()=>{
                const metrics = [
                  ...
                  { key:"sleep_hrs", label:"Sleep",    icon:"😴", unit:"hrs",   max:fitnessGoals.sleep_hrs, color:"#fdd835" },
